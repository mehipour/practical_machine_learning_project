...
## Feature Selection
First we randomly select 2000 data points from the training dataset to perform a prelimiarny analysis for feature selection. We apply a decision tree model on this data and assess the importance of each variable as predictors.
```{r feature selection 1}
set.seed(2)
idx <- sample(1:dim(training1)[1],2000)
...
```
We see that the majority of the predictors have low importance, therefore, we only keep predictors with a variable importance of at least 10% of the maximum value. Here we have a list of the predictors. This would leave us with 27 predicotrs.

```{r feature selection 2}
important_vars_idx <- var_importance_idx[which(var_importance > 0.1* max(var_importance))]
